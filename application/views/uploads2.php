...
<!--CONTENT START HERE-->
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			Import Man Power Expenses<!--<small>statistics and more</small> -->
			</h3>
			<div class="page-bar">
				<ul class="page-breadcrumb">
					<li>
						<i class="fa icon-user"></i>
						<a href="/uploads2/">Import Man Power</a>
...
function load_data(){
	...
	var date_to = $('#date_to').val();
	   ...
	   $.ajax({
		   url:"<?php echo base_url();?>man_power/expences_list2",
		   method:"post",
...
function load_data2(){
	...
	var date_to = $('#date_to').val();
	   ...
	   $.ajax({
		   url:"<?php echo base_url();?>man_power/expences_list_filter2",
		   method:"post",
...
function Upload() {
    ...
    $.ajax({
        type: "POST",
        url:"<?php echo base_url();?>uploads2/import",
		processData: false,
		contentType: false,
        data: formData,
        ...
        success : function(text) {
			...
			if ( text == "success" ) {
				...
				$( "#message" ).html("<font color =green><b>File Upload Success...</b></font>");
				
				toastr.success('File had been Uploaded', 'Data Added');
				$('#btn_save').attr('disabled',false);
                $('#my_file').attr('disabled',false).val('');
                $('#site_code').attr('disabled',false).val('');
                $('#show_loading').hide();
				$('#add_label').show();
			...
			else if(text =="check_alias"){
				$( "#message" ).html("<font color =red><b>Upload Error, Please recheck the file with corresponding code</b></font>");
				toastr.error('IMPORT ERROR PLEASE CHECK ALIAS ON YOU XLS OR CSV FILE', 'Error');
				$('#my_file').focus();
				$('#show_loading').hide();
				$('#btn_close').show();
				$('#add_label').show();
